...
public class CheckIsPalindromeWithSpansTests
{
    private readonly PalindromesChecker _palindromesChecker;
    public CheckIsPalindromeWithSpansTests()
    {
    ...
    {
        var result = _palindromesChecker.CheckIsPalindromeWithSpans(lowerCaseOnlyReadableSignsTextToCheck);
        Assert.True(result);
    }
    [Theory]
    [InlineData("")]
